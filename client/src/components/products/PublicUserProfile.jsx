...
export default function PublicUserProfile() {
    ...
    <section className="py-10 px-6 mt-8 bg-white dark:bg-[#1a1f2b] rounded-xl shadow-md transition-all duration-300 max-w-5xl mx-auto">
        ...
        <div className="space-y-4 text-lg text-gray-700 dark:text-gray-300">
            <p>
                <span className="font-semibold text-gray-900 dark:text-gray-400">Email:</span>{' '}
                {user.email}
            </p>
            <p>
                <span className="font-semibold text-gray-900 dark:text-gray-400">Description:</span>{' '}
                {user.description}
            </p>
            <p>
                <span className="font-semibold text-gray-900 dark:text-gray-400">Contacts:</span>{' '}
                {user.contacts}
            </p>
        </div>
    </section>
        ...
        {productsWithComments.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 max-w-7xl mx-auto">
                {productsWithComments.map((product) => {
                    const paginatedComments = getPaginatedComments(product);
                    return (
                        <div
                            key={`+${product.id}`}
                        ...
                        >
                            <div className="overflow-hidden rounded-t-2xl">
                                <Link to={`/products/selected/${product.id}`}>
                                    <img
                            ...
                            <div className="p-5">
                                ...
                                <div className="flex items-center gap-1 mb-3">
                                    {[...Array(5)].map((_, i) => {
                                        if (i < Math.floor(product.avgRating)) {
                                            return (
                                                <FaStar key={nanoid(64)} className="text-yellow-400" />
                                            );
                                        } else if (
                                            ...
                                            return (
                                                <FaStarHalf key={nanoid(64)} className="text-yellow-400" />
                                            );
                                        } else {
                                            return (
                                                <FaStar key={nanoid(64)} className="text-gray-300" />
                                            );
                                        }
                                    })}
                                ...
                                <div className="flex justify-between items-center mt-6">
                                    <button
                                        disabled={(commentPage[product.id] || 1) === 1}
                                        onClick={() => handlePrevPage(product.id)}
                                    ...
                                    <button
                                        disabled={
                                            (commentPage[product.id] || 1) * COMMENTS_PER_PAGE >=
                                            product.comments.length
                                        }
                                        onClick={() =>
                                            handleNextPage(product.id, product.comments.length)
                                        }
                                        className="text-sm font-medium text-indigo-600 hover:underline disabled:text-gray-400"
                                    >
                                        Next ▶
