        ...
        const fetchCategories = async () => {
            try {
                ...
                if (Array.isArray(categoriesData)) {
                    ...
                    console.error('API response is not an array:', categoriesData);
                    setCategories([]);
                    ...
                    <div className="flex flex-col md:flex-row p-4 space-y-4 md:space-y-0 md:space-x-4 border border-gray-200 rounded-md">
                        <ul className="w-full md:w-1/2 space-y-4">
                            {categories && categories.length > 0 ? (
                                            ...
                                            >
                                                <ul className="p-4 space-y-2 rounded-md">
                                                    {category.subcategories.length > 0 ? (
                                                        ...
                                                        <li className="px-4 py-2 text-lg text-gray-500">No subcategories</li>
                                                    )}
                                                </ul>
                                            </div>
                                        )}
                                ...
                                <li className="px-4 py-2 text-lg text-gray-500">No categories found</li>
                            )}
                        </ul>
                    </div>
                </div>
