...
const AdminPanel = () => {
  ...
  const filteredEvents = events.filter((event) => {
    ...
    return (
      ...
      new Date(event.timestamp).toLocaleString().toLowerCase().includes(term)
    );
  });
        ...
        <form
          onSubmit={handleSubmit(async (data) => {
            try {
              ...
              const hashedPassword = sha256(sha1(data.password));
      ...
      {activeTab === "events" && (
        <div className="bg-white dark:bg-gray-800 max-h-[25cm] p-4 rounded-lg shadow-lg">
          ...
          {events.length > 0 ? (
              ...
              <table className="min-w-full text-sm text-left">
                <thead className="border-b border-gray-300 dark:border-gray-600">
                  <tr>
                    ...
                    <th className="px-4 py-2">Naudotojas</th>
                ...
                <tbody>
                  {filteredEvents.map((event) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        {event.eventType?.name || event.type_id}
                      </td>
                      <td className="px-4 py-2">
                        {event.description?.startsWith("ADDENDUM")
                          ? "transaction"
                          : event.eventType?.name || event.type_id}
                      </td>
            ...
            <p className="text-center text-gray-400 py-4">Įvykių nėra</p>
          )}
        </div>
      )}
    </div>
