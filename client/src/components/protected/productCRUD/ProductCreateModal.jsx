...
const ProductCreateModal = ({ showModal, setShowModal }) => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const strippedFileTypes = fileTypes.map((fileType) => fileType.split("/")[1].toUpperCase()).join(", ");
        ...
        <form onSubmit={handleSubmit(submitHandler)} className="flex flex-col">
          ...
          <div>
            <input
              ...
              multiple
              {...register("images", {
                required: "Add at least one image of your product",
                onChange: () => {
                  setError("");
                  clearErrors("images");
                },
                validate: {
                  ...
                  fileType: (value) => {
                    if (!value[0]) return true;
                    return (
                      availableFileTypes.includes(value[0].type) ||
