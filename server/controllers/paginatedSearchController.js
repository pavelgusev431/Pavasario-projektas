...
import { filterItemsByRange, sortHelper } from './paginatedProductController.js'; // reuse your helpers

export async function searchProductsPaginated(req, res) {
  try {
    const { q } = req.query;
    let { page = 1, limit = 8, minPrice, maxPrice, minDate, maxDate, sort, order } = req.query;
    ...
    const events   = await Event.findAll({ where: { type_id: 1, target_id: 2 } });
    ...
    const withExtras = products.map((product) => {
      const evts = events.filter(
        (e) => e.user_id === product.user_id && e.product_id === product.id
      );
      const latest = evts.length
        ? evts.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp))[0]
        : null;

      const prodRatings = ratings.filter((r) => r.product_id === product.id);
    ...
    if (minPrice != null || maxPrice != null) {
      filtered = await filterItemsByRange(
        minPrice, maxPrice, filtered, null, null, 'price'
      );
    }
    if (minDate != null || maxDate != null) {
      filtered = await filterItemsByRange(
        minDate, maxDate, filtered, null, null, 'date'
      );
    }
