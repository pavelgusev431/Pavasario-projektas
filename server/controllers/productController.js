...
const getHotProducts = async (req, res, next) => {
  try {
    ...
    const processedProducts = products.map((product) => {
      ...
      const avgRating =
        ratingCount > 0
          ? productRatings.reduce((sum, rating) => sum + rating.stars, 0) /
            ratingCount
          : 0;
...
const getTopUserProducts = async (req, res, next) => {
  try {
    ...
    Object.entries(userStats).forEach(([userId, stats]) => {
      const avgUserRating =
        stats.totalRatings > 0 ? stats.totalStars / stats.totalRatings : 0;
...
const getTrendingUserProducts = async (req, res, next) => {
  try {
    ...
    if (newUserIds.length === 0) {
      return res.status(200).json({ message: "users not found", data: [] });
    }
    ...
    if (products.length === 0) {
      return res.status(200).json({ message: "Nerasta produktų", data: [] });
    }
...
const getRatedProductsByUserName = async (req, res) => {
  try {
    ...
    const processedProducts = userRatings.map((rating) => {
      ...
      const event =
        productEvents.find((e) => e.product_id === rating.product_id) || null;
...
const editProduct = async (req, res, next) => {
  try {
    ...
    if (!foundProduct) {
      ...
      throw new AppError("Forbidden to change other user's products", 403);
    } else {
...
const deleteProduct = async (req, res, next) => {
  try {
    ...
    if (!foundProduct) {
      ...
      throw new AppError("Forbidden to delete other user's products", 403);
    } else {
